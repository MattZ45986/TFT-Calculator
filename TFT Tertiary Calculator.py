...
sixthTier = []
fin = open("sixthTier.txt", "r")
for line in fin.readlines():
    team = line.strip().split()
    for i in range(1,9):
        team.pop(i)
    sixthTier.append(Team([eval(team[0].lower()),eval(team[1].lower()),eval(team[2].lower()),eval(team[3].lower()),eval(team[4].lower()),eval(team[5].lower())], int(team[6]),int(team[7]),int(team[8])))
fin.close()
...
########## NINTH TIER CALCULATION ############

print("0 percent done making tier 9")
ninthTier = []
startTime = time.time()
for k in range(10000):
    for i in range((len(sixthTier)*k)//10000,(len(sixthTier)*(k+1))//10000):
        for j in range(len(thirdTier)):
            team1 = sixthTier[i]
            team2 = thirdTier[j]
            if not areOverlapping(team1, team2) and haveTeamSynergy(team1, team2):
                ninthTier.append(Team(team1.champs+team2.champs))
    print(str((k+1)/100) + " percent done. " + str(time.time()-startTime) + " seconds elapsed")
    if k%100 == 0:
        for team in ninthTier:
            team.getLinks()
            team.getAffinities()
            team.getScore()
        print("population of batch #" + str(k//100) + " done.")
        fout = open("ninthTier"+str(k//100)+".txt","w+")
        for item in ninthTier:
            fout.write(str(item)+"\n")
        fout.close()
        print("output of batch #" + str(k//100) + " done.")
    ninthTier = []
